...
class SabreLayout(BasePass):
    ...
    def __init__(self, coupling_list: List = None,
        ...
        Args:
            ...
            initial_layout_method (str): Method used to select the exact-size connected
                subgraph when n < N: ``random``, ``fidelity`` or ``dense``.
        """
        super().__init__()
    ...
    def set_model(self, model):
        ...
        # If a resource manager has already reserved a region, select the exact-n
        # subgraph inside it. Otherwise select from the complete device graph.
    ...
    def run(self, circ_dag: Union[QuantumCircuit, DAGCircuit]):
        ...
        if self.sabre_initial_layout is not None:
            ...
            self._set_exact_used_subgraph(self.sabre_initial_layout, len(dag.qubits_used))
        ...
        if self.model.get_layout()["initial_layout"] is None:
            if len(dag.qubits_used) == self.coupling_graph.num_qubits:
                ...
                layout_types = {
                    'random': OverallLayoutRandom,
                    'fidelity': OverallLayoutFidelity,
                    'dense': OverallLayoutDense,
                }
                try:
                    layout_type = layout_types[self.initial_layout_method]
                except KeyError as exc:
                    raise ValueError(
                        "initial_layout_method can only be 'random', 'fidelity' or 'dense'."
                    ) from exc
                layout = layout_type(
                ...
                used_subgraph = self.coupling_graph.subgraph(selected_subgraph.nodes())
                self.model.set_layout({'initial_layout': layout})
